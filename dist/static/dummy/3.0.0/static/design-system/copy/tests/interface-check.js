...
const exclusions = {
    ...
    gridOptions: ['xl', 'offsetXl', 'lg', 'offsetLg', 'md', 'offsetMd', 'sm', 'offsetSm', 'xs', 'offsetXs'],
    maskOptions: ['mask', 'guide', 'keepCharPositions', 'placeholderChar', 'pipe', 'onBeforeChange'],
    numberOptions: ['decimalLimit', 'prefix', 'suffix', 'allowDecimal', 'allowNegative', 'allowEmpty',
        'includeThousandsSeparator', 'thousandsSeparatorSymbol', 'decimalSymbol', 'currency'],
    ...
    inputSuggestOptions: ['isLoading', 'inputComponent', 'query', 'options', 'optionsLabel', 'isSearch', 'iconFromCode', 'onChangeOption', 'onChangeInput', 'onRetry', 'onScroll'],
    inputSuggestOptionOptions: ['searchString', 'iconFromCode', 'fallbackIcon'],
    inputSuggestInputOptions: ['fallbackIcon', 'iconFromCode', 'inputComponent', 'withOptions'],
    inputSuggestDynamicOptions: ['initialQuery', 'initialOptions', 'masked', 'keyboardTimeout', 'requestTimeout', 'onDataRequest', 'onChangeOption', 'onChangeInput', 'options', 'requestMode', 'withPagination'],
    timeoutOptions: ['keyboardTimeout', 'requestTimeout'],

    Icon: ['onError']
}

const isValidProp = (propName, componentExclusions, componentPropType) => _.includes(complexTypes, propName)
...
export const interfaceCheck = (exports, componentsWithExclusions, parentPath) => {
    if (!parentPath) {
        interfaceCheck.warnList = []
    }

    _.forEach(exports, (Component, name) => {
        if (interfaceCheck.componentRegExp.test(name) && name !== interfaceCheck.decoratorsShortcut) {
            const path = parentPath ? `${parentPath}.${name}` : name

            _.forEach(Component.propTypes, (componentPropType, propName) => {
                if (!isValidProp(propName, componentsWithExclusions[path], componentPropType)) {
                    interfaceCheck.warnList.push(`Prop ${propName} компонента ${Component.displayName || Component.name}`)
                }
            })
