...
import '../../constants.js';
import { isSameYear, startOfYear, endOfYear, toDate, setYear, format } from 'date-fns';
...
var Cell = function Cell(_ref) {
  ...
  var isActive = isSameYear(year, currentDate);
  var isToday = isSameYear(year, new Date());
  var isDisabled = !restriction(year, {
    start: startOfYear(year),
    end: endOfYear(year)
  });
  var handleButtonClick = useCallback(function (e) {
    var yearDate = toDate(e.target.value);
    var changedShowDate = fullISOFormat(setYear(showDate, format(yearDate, 'YYYY')));
    handleClick(changedShowDate, e);
  }, []);
  return /*#__PURE__*/React.createElement("td", {
    className: commonTableStyles.tableCell
  }, /*#__PURE__*/React.createElement(SquareButton, {
    className: classnames(style.item, isToday && commonStyles.itemNow, isActive && commonStyles.itemActive),
    onClick: handleButtonClick,
    disabled: isDisabled,
    value: year,
    dataUnit: "calendar:years:year".concat(isActive ? ':active' : '')
  }, format(year, 'YYYY')));
};
