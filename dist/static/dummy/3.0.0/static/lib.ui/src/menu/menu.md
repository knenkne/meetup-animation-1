```jsx
{
    var options = [
        ...
        {
            value: "#",
            title: "Почитать договор",
            type: "link",
            action: (event = {}) => setState({ text: "Клик по ссылке", eventType: event.type })
        },
        {
            value: "2rows",
            title: "Сделать что-то достаточно длинное",
            type: "option",
            action: (event = {}) => setState({ text: "Сделать длинное дело", eventType: event.type })
        }
    ]
