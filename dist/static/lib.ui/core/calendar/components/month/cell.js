...
import { startOfDay, endOfDay, isSameDay, setHours, setMinutes, getDate } from 'date-fns';
...
var Cell = function Cell(_ref) {
  ...
  var value = fullISOFormat(date);
  var isDisabled = !restriction(date, value && {
    start: startOfDay(value),
    end: endOfDay(value)
  });
  var isActive = isSameDay(date, currentDate);
  var handleButtonClick = useCallback(function (e) {
    if (!isDisabled) {
      var passedDate = fullISOFormat(setHours(setMinutes(date, DEFAULT_DATE.MINUTES), DEFAULT_DATE.HOURS));
      handleClick(passedDate, e);
    }
  }, []);
  return /*#__PURE__*/React.createElement("td", {
    className: classnames(!date && style.noPadding, !isDisabled && style.pointer)
  }, date ? /*#__PURE__*/React.createElement("button", {
    onMouseDown: handleButtonClick,
    className: classnames(style.item, isActive && style.itemActive, isSameDay(date, new Date()) && style.itemNow),
    tabIndex: "-1",
    disabled: isDisabled,
    "data-text": getDate(date),
    "data-unit": "calendar:month:day",
    type: "button"
  }, getDate(date)) : /*#__PURE__*/React.createElement("div", {
    className: classnames(style.itemEmpty)
  }));
};
Cell.propTypes = {
